...
def main(args):
    ...
    if args.lock_image:
        ...
        if hasattr(model.visual, 'proj'):
            print('WORKAROUND: locking image tower but not proj')
    ...
    if is_master(args):
        ...
        with open(params_file, "w") as f:
            for name in sorted(vars(args)):
                ...
                f.write(f"{name}: {val}\n")
        config_file = os.path.join(args.logs, args.name, 'config.json')
        model_cfg_to_record = get_model_config(args.model)
        with open(config_file, 'w') as f:
            json.dump(model_cfg_to_record, f)
