...
def parse_args(args):
    ...
    parser.add_argument(
        "--csv-dataset-sample-type",
        default="default",
        type=str,
        help='CSVDataset use default or sampled category for ranking',
    )
    parser.add_argument(
        "--negative-ratio",
        default=0.2,
        type=float,
        help="Dataloader contains negative samples (CSV format only) with ratio."
    )
    parser.add_argument(
        "--train-with-relevance-score",
        action="store_true",
        help='Reference to a random target score instead of 0.',
    )
    parser.add_argument(
        "--loss-opt",
        choices=["single-head", "dual-head"],
        default="single-head",
        type=str,
        help="Control loss options",
    )
    parser.add_argument(
        "--loss-components",
        nargs="+",
        default=["rank"],
        help="Loss Component (rank, regression, bce)",
    )
