...
from .clip_model_adapter import L2RCLIP
from .loss import (
    ClipLoss,
    DistillClipLoss,
    CoCaLoss,
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        if custom_text:
            if "multimodal_cfg" in model_cfg:
                if "adapter_cfg" in model_cfg:
                    print("Use Custom L2RCoCa")
                    model = L2RCoCa(**model_cfg, cast_dtype=cast_dtype)
                else:
            ...
            if 'adapter_cfg' in model_cfg:
                if 'misc_cfg' not in model_cfg:
                    model_version = 'clip-adapter-v1'
                else:
                    model_version = model_cfg['misc_cfg']['model_version']

                print(f'Use Custom L2RCLIP {model_cfg["misc_cfg"]["model_version"]}')
                model = L2RCLIP(
                    **model_cfg,
...
def create_loss(args):
    if args.distill:
        ...
        if "adapter" in args.model.lower():
            # TODO: Not a good idea
            print('Use CoCaAdapterLoss')
            return CoCaWithAdapterLoss(
    ...
    if 'adapter' in args.model.lower():
        if args.loss_opt == 'single-head':
            print('Use ClipAdapterLoss')
            return ClipWithAdapterLoss(
                local_loss=args.local_loss,
                gather_with_grad=args.gather_with_grad,
                cache_labels=True,
                rank=args.rank,
        ...
        elif args.loss_opt == 'dual-head':
            return ClipAdapterLossV2(
                local_loss=args.local_loss,
                gather_with_grad=args.gather_with_grad,
                cache_labels=True,
                rank=args.rank,
    ...
    if 'rankdr' in args.model.lower():
        print('Rank Direct, use ClipDirectRankLoss')
        return ClipDirectRankLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
            rank=args.rank,
